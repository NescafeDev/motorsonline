  ...
  try {
    ...
    if (reqWithFiles.files) {
      if (reqWithFiles.files['title_image']) {
        data.title_image = `/img/${reqWithFiles.files['title_image'][0].filename}`;
      }
      if (reqWithFiles.files['intro_image']) {
        data.intro_image = `/img/${reqWithFiles.files['intro_image'][0].filename}`;
      }
    }
    const blog = await createBlog(data);
    if (reqWithFiles.files) {
      const blogId = blog.id;
      const blogDir = path.join('/img', String(blogId));
      ...
      for (const field of ['title_image', 'intro_image']) {
        if (reqWithFiles.files[field]) {
          ...
          data[field] = `/img/${blogId}/${field}${ext}`;
        }
      }
  ...
  if (reqWithFiles.files) {
    for (const field of ['title_image', 'intro_image']) {
      if (reqWithFiles.files[field]) {
        ...
        const blogDir = path.join('/img', String(id));
        if (!fs.existsSync(blogDir)) fs.mkdirSync(blogDir, { recursive: true });
        const newPath = path.join(blogDir, `${field}${ext}`);
